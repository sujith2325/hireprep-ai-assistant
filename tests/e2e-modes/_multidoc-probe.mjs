...
// wait ready
for(let i=0;i<25;i++){ const st=await R('__e2e__:index-status',modeId); if((st?.statuses||[]).length>=3 && st.statuses.every(s=>s.status==='ready'||s.status==='lexical_only')){console.log('ready after',i,'s',JSON.stringify(st.statuses.map(s=>s.status+':'+s.chunkCount)));break;} await new Promise(r=>setTimeout(r,1000)); }
await R('__e2e__:prewarm-mode',modeId); await new Promise(r=>setTimeout(r,1500));
// inspect retrieval for the failing BLEU question
const insp=await R('__e2e__:inspect-retrieval',{modeId,query:'BLEU score WMT 2014 English German Transformer big model',forceDocumentGrounding:true});
console.log('retrieval blockLen:',insp?.blockLength,'contains 28.4:', /28\.4/.test(insp?.block||''));
console.log('block sample:', (insp?.block||'').slice(0,300).replace(/\n/g,' '));
