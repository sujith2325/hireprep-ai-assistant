...
const App: React.FC = () => {
  ...
  const [isPhoneShellActive, setIsPhoneShellActive] = useState<boolean>(() => {
    try { return localStorage.getItem('hireprep_phone_shell_active') === 'true'; } catch { return false; }
  });

  const togglePhoneShell = useCallback(() => {
    setIsPhoneShellActive(prev => {
      const next = !prev;
      try { localStorage.setItem('hireprep_phone_shell_active', String(next)); } catch {}
      return next;
    });
  }, []);
  ...
  return (
    ...
    <div className="h-full min-h-0 w-full relative bg-transparent">
      ...
      <div data-opacity-preview-surface="">
        ...
        {/* Main Launcher View (Search + History) */}
        {!isolateGlobalSurfaces && (isLauncherWindow || isDefault) && (
          isPhoneShellActive ? (
            <PhoneShell title="HirePrep Mobile Shell">
              <Launcher
                onStartMeeting={() => window.electronAPI?.openMeetingWindow()}
                onOpenSettings={(tab?: string) => openSettingsExclusive(tab)}
                onOpenProfile={() => openProfileExclusive()}
                onOpenModes={() => openModesExclusive()}
                onPageChange={(isMain) => setIsLauncherMainView(isMain)}
                onTogglePhoneShell={togglePhoneShell}
                isPhoneShellActive={isPhoneShellActive}
                ollamaPullStatus={ollamaPullStatus}
                ollamaPullPercent={ollamaPullPercent}
                ollamaPullMessage={ollamaPullMessage}
              />
